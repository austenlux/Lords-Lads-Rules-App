...
5a. **UI/Layout regression checklist (for any UI or layout change).** Read `docs/project-context.md` — it contains the project's UI regression checklist with the specific screens, components, and visual invariants that must be verified on every UI change. Run each item on iOS (and Android where parity is expected). Report pass/fail for each. If no checklist is defined in project context, derive regression items from the feature's acceptance criteria and any UI invariants visible in the existing codebase.
